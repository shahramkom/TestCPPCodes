...
#pragma NDIS_INIT_FUNCTION(ShNdisFilterDriverRegisterDevice)


_IRQL_requires_max_(PASSIVE_LEVEL)
NDIS_STATUS
ShNdisFilterDriverRegisterDevice(
    VOID
    )
{
    ...
    DEBUGP(DL_TRACE, "==>ShNdisFilterDriverRegisterDevice\n");
   
    
    NdisZeroMemory(DispatchTable, (IRP_MJ_MAXIMUM_FUNCTION+1) * sizeof(PDRIVER_DISPATCH));
    
    DispatchTable[IRP_MJ_CREATE] = ShNdisFilterDriverDispatch;
    DispatchTable[IRP_MJ_CLEANUP] = ShNdisFilterDriverDispatch;
    DispatchTable[IRP_MJ_CLOSE] = ShNdisFilterDriverDispatch;
    DispatchTable[IRP_MJ_DEVICE_CONTROL] = ShNdisFilterDriverDeviceIoControl;
    ...
    DEBUGP(DL_TRACE, "<==ShNdisFilterDriverRegisterDevice: %x\n", Status);
...
_IRQL_requires_max_(PASSIVE_LEVEL)
VOID
ShNdisFilterDriverDeregisterDevice(
    VOID
    )
...
_Use_decl_annotations_                
NTSTATUS
ShNdisFilterDriverDeviceIoControl(
    PDEVICE_OBJECT        DeviceObject,
    PIRP                  Irp
    )
{
